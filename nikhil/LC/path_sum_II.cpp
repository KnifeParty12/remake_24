...
public:
               ...
               vector<vector<int>> &res)
    {
        ...
        // not a leaf
        // can be part of the answer and cannot be also
        // assume it is
        curr.pb(root->val);
        if (root->left)
            solve(root->left, targetSum - root->val, curr, res);
        if (root->right)
            solve(root->right, targetSum - root->val, curr, res);
        curr.pop_back();
    }

    vector<vector<int>> pathSum(TreeNode *root, int targetSum)
    {
        // need to return root to leaf paths only
        vector<vector<int>> res = {};
        vector<int> temp = {};
